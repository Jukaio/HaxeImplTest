/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.componentvisualizers;

/**
  WARNING: This type was not defined as DLL export on its declaration. Because of that, some of its methods are inaccessible
  
  
**/
@:umodule("ComponentVisualizers")
@:glueCppIncludes("SplineGeneratorPanel.h")
@:noClass @:uextern @:uclass extern class ULineSplineGenerator extends unreal.componentvisualizers.USplineGeneratorBase {
  
  /**
    If enabled, will add points up until the next existing point
  **/
  @:uproperty public var bUpToNextPoint : Bool;
  @:uproperty public var bEnableUpToNextPoint : Bool;
  @:uproperty public var Length : unreal.Float32;
  @:uproperty public var NumberOfPoints : unreal.Int32;
  
}
