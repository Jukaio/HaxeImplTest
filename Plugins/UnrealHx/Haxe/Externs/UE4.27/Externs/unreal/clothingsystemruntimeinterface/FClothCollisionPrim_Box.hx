/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.clothingsystemruntimeinterface;

/**
  Data for a single box primitive.
**/
@:umodule("ClothingSystemRuntimeInterface")
@:glueCppIncludes("Public/ClothCollisionPrim.h")
@:noCopy @:noEquals @:uextern @:ustruct extern class FClothCollisionPrim_Box {
  @:uproperty public var BoneIndex : unreal.Int32;
  @:uproperty public var HalfExtents : unreal.FVector;
  @:uproperty public var LocalRotation : unreal.FQuat;
  @:uproperty public var LocalPosition : unreal.FVector;
  
}
