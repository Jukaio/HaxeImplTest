/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.gameplaycameras;

/**
  A base class for a simple camera shake.
**/
@:umodule("GameplayCameras")
@:glueCppIncludes("CompositeCameraShakePattern.h")
@:uextern @:uclass extern class UCompositeCameraShakePattern extends unreal.UCameraShakePattern {
  
  /**
    The list of child shake patterns
  **/
  @:uproperty public var ChildPatterns : unreal.TArray<unreal.UCameraShakePattern>;
  
}
