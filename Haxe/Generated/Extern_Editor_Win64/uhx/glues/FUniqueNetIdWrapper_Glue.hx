// Ver:1
// GeneratedBy:c:\users\jukai\documents\unreal projects\haxetest\plugins\unrealhx/haxe/externs/ue4.27/unreal/funiquenetidwrapper.hx
/*******************************************************
 * 
 * WARNING! This file was autogenerated by: 
 *  _    _                      _   _ 
 * | |  | |                    | | | |
 * | |  | |_ __  _ __ ___  __ _| | | |__ __  __
 * | |  | | '_ \| '__/ _ \/ _` | | | '_ \ \/ /
 * | |__| | | | | | |  __/ (_| | |_| | | |>  < 
 *  \____/|_| |_|_|  \___|\__,_|_(_)_| |_/_/\_\
 *******************************************************/


package uhx.glues;
@:unrealGlue extern class FUniqueNetIdWrapper_Glue {
public static function create():unreal.VariantPtr;
public static function createFromIdRef(InUniqueNetId:unreal.VariantPtr):unreal.VariantPtr;
public static function createFromIdPtr(InUniqueNetId:unreal.VariantPtr):unreal.VariantPtr;
public static function ToString(self:unreal.VariantPtr):unreal.VariantPtr;
public static function ToDebugString(self:unreal.VariantPtr):unreal.VariantPtr;
public static function IsValid(self:unreal.VariantPtr):Bool;
public static function SetUniqueNetId(self:unreal.VariantPtr, InUniqueNetId:unreal.VariantPtr):Void;
public static function GetUniqueNetId(self:unreal.VariantPtr):unreal.VariantPtr;
public static function copyNew(self:unreal.VariantPtr):unreal.VariantPtr;
public static function copy(self:unreal.VariantPtr):unreal.VariantPtr;
public static function assign(self:unreal.VariantPtr, val:unreal.VariantPtr):Void;
public static function equals(self:unreal.VariantPtr, other:unreal.VariantPtr):Bool;
}