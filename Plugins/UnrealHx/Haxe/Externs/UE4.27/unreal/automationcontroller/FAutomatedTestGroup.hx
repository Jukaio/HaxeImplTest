/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.automationcontroller;

/**
  *     Describes a group of tests. Each group has a name and a set of filters that determine group membership
**/
@:umodule("AutomationController")
@:glueCppIncludes("Public/AutomationControllerSettings.h")
@:noCopy @:noEquals @:uextern @:ustruct extern class FAutomatedTestGroup {
  @:uproperty public var Filters : unreal.TArray<unreal.automationcontroller.FAutomatedTestFilter>;
  @:uproperty public var Name : unreal.FString;
  
}
