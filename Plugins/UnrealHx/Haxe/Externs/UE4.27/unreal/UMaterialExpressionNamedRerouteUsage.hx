/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal;

@:glueCppIncludes("Materials/MaterialExpressionNamedReroute.h")
@:uextern @:uclass extern class UMaterialExpressionNamedRerouteUsage extends unreal.UMaterialExpressionNamedRerouteBase {
  
  /**
    The variable GUID, to support copy across graphs
  **/
  @:uproperty public var DeclarationGuid : unreal.FGuid;
  
  /**
    The declaration this node is linked to
  **/
  @:uproperty public var Declaration : unreal.UMaterialExpressionNamedRerouteDeclaration;
  
}
