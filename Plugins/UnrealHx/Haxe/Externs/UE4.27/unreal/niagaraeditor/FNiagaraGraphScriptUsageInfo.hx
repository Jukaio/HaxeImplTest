/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.niagaraeditor;

/**
  Container for UNiagaraGraph cached data for managing CompileIds and Traversals.
**/
@:umodule("NiagaraEditor")
@:glueCppIncludes("Public/NiagaraGraph.h")
@:noCopy @:noEquals @:uextern @:ustruct extern class FNiagaraGraphScriptUsageInfo {
  
  /**
    The traversal of output to input nodes for this graph. This is not a recursive traversal, it just includes nodes from this graph.
  **/
  @:uproperty public var Traversal : unreal.TArray<unreal.niagaraeditor.UNiagaraNode>;
  @:uproperty public var CompileLastObjects : unreal.TArray<unreal.niagara.FNiagaraCompileHashVisitorDebugInfo>;
  
  /**
    The hash that we calculated last traversal.
  **/
  @:uproperty public var CompileHashFromGraph : unreal.niagaracore.FNiagaraCompileHash;
  
  /**
    The hash that we calculated last traversal.
  **/
  @:uproperty public var CompileHash : unreal.niagaracore.FNiagaraCompileHash;
  
  /**
    The particular instance of the usage type. Event scripts, for example, have potentially multiple graphs.
  **/
  @:uproperty public var UsageId : unreal.FGuid;
  
  /**
    The context in which this sub-graph traversal will be used.
  **/
  @:uproperty public var UsageType : unreal.niagara.ENiagaraScriptUsage;
  
  /**
    A guid which is generated when this usage info is created.  Allows for forced recompiling when the cached ids are invalidated.
  **/
  @:uproperty public var BaseId : unreal.FGuid;
  
}
