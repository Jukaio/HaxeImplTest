...
public:
  FHaxeGenerator(FHaxeTypes& inTypes) :
    m_buf(FHelperBuf()),
    m_haxeTypes(inTypes)
  {
  }
...
protected:
  bool writeWithModifiers(const FString &inName, FProperty *inProp, FString &outType);

  bool writeBasicWithModifiers(const FString &inName, FProperty *inProp, FString &outType);
  ...
  bool upropType(FProperty* inProp, FString &outType);
  bool isReadOnly(FProperty* inProp);
