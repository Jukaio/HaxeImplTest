/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.niagara;

/**
  Scalability settings for Niagara Emitters on a particular platform set.
**/
@:umodule("Niagara")
@:glueCppIncludes("Classes/NiagaraEffectType.h")
@:noCopy @:noEquals @:uextern @:ustruct extern class FNiagaraEmitterScalabilitySettings {
  
  /**
    Scale factor applied to spawn counts for this emitter.
  **/
  @:uproperty public var SpawnCountScale : unreal.Float32;
  
  /**
    Enable spawn count scaling
  **/
  @:uproperty public var bScaleSpawnCount : Bool;
  
  /**
    The platforms on which these settings are active (unless overridden).
  **/
  @:uproperty public var Platforms : unreal.niagara.FNiagaraPlatformSet;
  
}
