/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.editor;

@:umodule("UnrealEd")
@:glueCppIncludes("Preferences/PersonaOptions.h")
@:uextern @:uclass extern class UPersonaOptions extends unreal.UObject {
  
  /**
    Whether to snap to various things
  **/
  @:uproperty public var TimelineEnabledSnaps : unreal.TArray<unreal.FName>;
  
  /**
    Whether to display keys in the timeline's curve tracks
  **/
  @:uproperty public var bTimelineDisplayCurveKeys : Bool;
  
  /**
    Whether to display secondary format (times/frames) in the anim timeline
  **/
  @:uproperty public var bTimelineDisplayFormatSecondary : Bool;
  
  /**
    Whether to display percentage in the anim timeline
  **/
  @:uproperty public var bTimelineDisplayPercentage : Bool;
  
  /**
    Display format for the anim timeline
  **/
  @:uproperty public var TimelineDisplayFormat : unreal.timemanagement.EFrameNumberDisplayFormats;
  
  /**
    Snap value used to determine scrub resolution of the anim timeline
  **/
  @:uproperty public var TimelineScrubSnapValue : unreal.Int32;
  
  /**
    Snap value used to determine scrub resolution of the curve timeline
  **/
  @:uproperty public var CurveEditorSnapInterval : unreal.Float32;
  
  /**
    Options that should be unique per asset editor (like skeletal mesh or anim sequence editors)
  **/
  @:uproperty public var AssetEditorOptions : unreal.TArray<unreal.editor.FAssetEditorOptions>;
  
  /**
    The number of folder filters to allow at any one time in the animation tool's asset browser
  **/
  @:uproperty public var NumFolderFiltersInAssetBrowser : unreal.FakeUInt32;
  
  /**
    Whether or not Skeletal Mesh Section selection should be enabled by default for the Animation Editor(s)
  **/
  @:uproperty public var bAllowMeshSectionSelection : Bool;
  @:uproperty public var bAllowPreviewMeshCollectionsToUseCustomAnimBP : Bool;
  @:uproperty public var bAllowPreviewMeshCollectionsToSelectFromDifferentSkeletons : Bool;
  
  /**
    Whether to hide parent items when filtering or to display them grayed out
  **/
  @:uproperty public var bHideParentsWhenFiltering : Bool;
  
  /**
    Whether to keep the hierarchy or flatten it when searching for bones, sockets etc.
  **/
  @:uproperty public var bFlattenSkeletonHierarchyWhenFiltering : Bool;
  
  /**
    Whether to use a socket editor that is created in-line inside the skeleton tree, or whether to use the separate details panel
  **/
  @:uproperty public var bUseInlineSocketEditor : Bool;
  @:uproperty public var BranchingPointTimingNodeColor : unreal.FLinearColor;
  @:uproperty public var NotifyTimingNodeColor : unreal.FLinearColor;
  @:uproperty public var SectionTimingNodeColor : unreal.FLinearColor;
  
  /**
    Index used to determine which Bone Draw Mode should be used by default for the Animation Editor(s)
  **/
  @:uproperty public var DefaultBoneDrawSelection : unreal.FakeUInt32;
  
  /**
    Index used to determine which ViewMode should be used by default for the Animation Editor(s)
  **/
  @:uproperty public var DefaultLocalAxesSelection : unreal.FakeUInt32;
  
  /**
    Currently Stats can have None, Basic and Detailed. Please refer to EDisplayInfoMode.
  **/
  @:uproperty public var ShowMeshStats : unreal.Int32;
  @:uproperty public var bUseAudioAttenuation : Bool;
  
  /**
    Whether or not audio should be muted by default for the Animation Editor(s)
  **/
  @:uproperty public var bMuteAudio : Bool;
  
  /**
    Whether or not the XYZ axis at the origin should be highlighted on the grid by default
  **/
  @:uproperty public var bHighlightOrigin : Bool;
  
  /**
    Whether or not the grid should be visible by default for the Animation Editor(s)
  **/
  @:uproperty public var bShowGrid : Bool;
  
  /**
    Whether or not the floor should be aligned to the Skeletal Mesh's bounds by default for the Animation Editor(s)
  **/
  @:uproperty public var bAutoAlignFloorToMesh : Bool;
  
}
