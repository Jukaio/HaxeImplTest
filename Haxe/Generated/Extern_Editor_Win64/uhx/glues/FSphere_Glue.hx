// Ver:1
// GeneratedBy:c:\users\jukai\documents\unreal projects\haxetest\plugins\unrealhx/haxe/externs/common/unreal/fsphere.hx
/*******************************************************
 * 
 * WARNING! This file was autogenerated by: 
 *  _    _                      _   _ 
 * | |  | |                    | | | |
 * | |  | |_ __  _ __ ___  __ _| | | |__ __  __
 * | |  | | '_ \| '__/ _ \/ _` | | | '_ \ \/ /
 * | |__| | | | | | |  __/ (_| | |_| | | |>  < 
 *  \____/|_| |_|_|  \___|\__,_|_(_)_| |_/_/\_\
 *******************************************************/


package uhx.glues;
@:unrealGlue extern class FSphere_Glue {
public static function createForceInit(ForceInit:Int):unreal.VariantPtr;
public static function createNewForceInit(ForceInit:Int):unreal.VariantPtr;
public static function create(InV:unreal.VariantPtr, InW:cpp.Float32):unreal.VariantPtr;
public static function createNew(InV:unreal.VariantPtr, InW:cpp.Float32):unreal.VariantPtr;
public static function get_Center(self:unreal.VariantPtr):unreal.VariantPtr;
public static function set_Center(self:unreal.VariantPtr, value:unreal.VariantPtr):Void;
public static function get_W(self:unreal.VariantPtr):cpp.Float32;
public static function set_W(self:unreal.VariantPtr, value:cpp.Float32):Void;
public static function copyNew(self:unreal.VariantPtr):unreal.VariantPtr;
public static function copy(self:unreal.VariantPtr):unreal.VariantPtr;
public static function assign(self:unreal.VariantPtr, val:unreal.VariantPtr):Void;
public static function equals(self:unreal.VariantPtr, other:unreal.VariantPtr):Bool;
}