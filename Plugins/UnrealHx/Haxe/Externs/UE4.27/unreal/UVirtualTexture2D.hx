/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal;

/**
  WARNING: This type was not defined as DLL export on its declaration. Because of that, some of its methods are inaccessible
  
  Virtual Texture with locally configurable build settings.
  A raw UTexture2D can also represent a Virtual Texture but uses the one and only per-project build settings.
**/
@:glueCppIncludes("VT/VirtualTexture.h")
@:noClass @:uextern @:uclass extern class UVirtualTexture2D extends unreal.UTexture2D {
  @:uproperty public var bSinglePhysicalSpace : Bool;
  @:uproperty public var bContinuousUpdate : Bool;
  @:uproperty public var Settings : unreal.FVirtualTextureBuildSettings;
  
}
