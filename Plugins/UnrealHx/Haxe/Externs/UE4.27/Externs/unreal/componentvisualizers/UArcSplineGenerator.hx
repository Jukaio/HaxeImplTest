/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.componentvisualizers;

/**
  WARNING: This type was not defined as DLL export on its declaration. Because of that, some of its methods are inaccessible
  
  
**/
@:umodule("ComponentVisualizers")
@:glueCppIncludes("SplineGeneratorPanel.h")
@:noClass @:uextern @:uclass extern class UArcSplineGenerator extends unreal.componentvisualizers.USplineGeneratorBase {
  
  /**
    If enabled, will switch the arc's center to the right of the curve
  **/
  @:uproperty public var bBranchRight : Bool;
  
  /**
    If enabled, will start the shape tangent to the current path
  **/
  @:uproperty public var bKeepFirstKeyTangent : Bool;
  
  /**
    If enabled, will reverse the direction of the arc
  **/
  @:uproperty public var bReverseDir : Bool;
  
  /**
    Degree of arc
  **/
  @:uproperty public var Degrees : unreal.Float32;
  
  /**
    Radius of arc
  **/
  @:uproperty public var Radius : unreal.Float32;
  
  /**
    Number of points making up shape
  **/
  @:uproperty public var NumberOfPoints : unreal.Int32;
  
}
