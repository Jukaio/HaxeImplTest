// Ver:1
// GeneratedBy:c:\users\jukai\documents\unreal projects\haxetest\plugins\unrealhx/haxe/externs/ue4.27/unreal/niagara/uniagarapreviewaxis_interpparambase.hx
/*******************************************************
 * 
 * WARNING! This file was autogenerated by: 
 *  _    _                      _   _ 
 * | |  | |                    | | | |
 * | |  | |_ __  _ __ ___  __ _| | | |__ __  __
 * | |  | | '_ \| '__/ _ \/ _` | | | '_ \ \/ /
 * | |__| | | | | | |  __/ (_| | |_| | | |>  < 
 *  \____/|_| |_|_|  \___|\__,_|_(_)_| |_/_/\_\
 *******************************************************/


package uhx.glues;
@:unrealGlue extern class UNiagaraPreviewAxis_InterpParamBase_Glue {
public static function get_Count(self:unreal.UIntPtr):Int;
public static function set_Count(self:unreal.UIntPtr, value:Int):Void;
public static function get_Param(self:unreal.UIntPtr):unreal.VariantPtr;
public static function set_Param(self:unreal.UIntPtr, value:unreal.VariantPtr):Void;
}