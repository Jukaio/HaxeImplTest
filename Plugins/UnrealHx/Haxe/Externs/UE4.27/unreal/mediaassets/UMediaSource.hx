/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.mediaassets;

/**
  Abstract base class for media sources.
  
  Media sources describe the location and/or settings of media objects that can
  be played in a media player, such as a video file on disk, a video stream on
  the internet, or a web cam attached to or built into the target device. The
  location is encoded as a media URL string, whose URI scheme and optional file
  extension will be used to locate a suitable media player.
**/
@:umodule("MediaAssets")
@:glueCppIncludes("MediaSource.h")
@:uextern @:uclass extern class UMediaSource extends unreal.UObject {
  
  /**
    Get the media source's URL string (must be implemented in child classes).
    
    @return The media URL.
    @see GetProxies
  **/
  @:ufunction(BlueprintCallable) @:thisConst public function GetUrl() : unreal.FString;
  
  /**
    Validate the media source settings (must be implemented in child classes).
    
    @return true if validation passed, false otherwise.
  **/
  @:ufunction(BlueprintCallable) @:thisConst public function Validate() : Bool;
  
  /**
    Set a boolean parameter to pass to the player.
  **/
  @:ufunction(BlueprintCallable) @:final public function SetMediaOptionBool(Key : unreal.Const<unreal.PRef<unreal.FName>>, Value : Bool) : Void;
  
  /**
    Set a float parameter to pass to the player.
  **/
  @:ufunction(BlueprintCallable) @:final public function SetMediaOptionFloat(Key : unreal.Const<unreal.PRef<unreal.FName>>, Value : unreal.Float32) : Void;
  
  /**
    Set an integer64 parameter to pass to the player.
  **/
  @:ufunction(BlueprintCallable) @:final public function SetMediaOptionInt64(Key : unreal.Const<unreal.PRef<unreal.FName>>, Value : unreal.Int64) : Void;
  
  /**
    Set a string parameter to pass to the player.
  **/
  @:ufunction(BlueprintCallable) @:final public function SetMediaOptionString(Key : unreal.Const<unreal.PRef<unreal.FName>>, Value : unreal.FString) : Void;
  
}
