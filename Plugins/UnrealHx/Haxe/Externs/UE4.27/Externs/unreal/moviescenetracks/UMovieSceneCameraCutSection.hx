/**
 * 
 * WARNING! This file was autogenerated by: 
 *  _   _ _   _ __   __ 
 * | | | | | | |\ \ / / 
 * | | | | |_| | \ V /  
 * | | | |  _  | /   \  
 * | |_| | | | |/ /^\ \ 
 *  \___/\_| |_/\/   \/ 
 * 
 * This file was autogenerated by UnrealHxGenerator using UHT definitions.
 * It only includes UPROPERTYs and UFUNCTIONs. Do not modify it!
 * In order to add more definitions, create or edit a type with the same name/package, but with an `_Extra` suffix
**/
package unreal.moviescenetracks;

/**
  WARNING: This type was defined as MinimalAPI on its declaration. Because of that, its properties/methods are inaccessible
  
  Movie CameraCuts are sections on the CameraCuts track, that show what the viewer "sees"
**/
@:umodule("MovieSceneTracks")
@:glueCppIncludes("Sections/MovieSceneCameraCutSection.h")
@:uextern @:uclass extern class UMovieSceneCameraCutSection extends unreal.moviescene.UMovieSceneSection implements unreal.moviescene.IMovieSceneEntityProvider {
  
  /**
    When blending, lock the previous camera (camera cut or gameplay camera).
  **/
  @:uproperty public var bLockPreviousCamera : Bool;
  // MovieSceneEntityProvider interface implementation
  
}
