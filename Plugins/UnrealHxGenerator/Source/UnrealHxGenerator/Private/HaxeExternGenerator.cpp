...
static TArray<FString> getFPropertyFlags(FProperty *prop) {
  TArray<FString> ret;
...
void FHaxeGenerator::generateFields(UStruct *inStruct, bool onlyProps = false) {
  ...
  while (fields.Num() > 0) {
    ...
    if (field->IsA<FProperty>()) {
      auto prop = CastField<FProperty>(field);
      if (prop->HasAnyPropertyFlags(CPF_Protected) && prop->IsA<FBoolProperty>()) {
      ...
      if ((prop->HasAnyFlags(RF_Public) || prop->HasAnyPropertyFlags(CPF_Protected)) && upropType(prop, type)) {
        ...
        auto flags = getFPropertyFlags(prop);
        if (flags.Num() != 0) {
...
bool FHaxeGenerator::generateDelegate(const DelegateDescriptor *inDelegate) {
  ...
  auto shouldExport = true;
  bool hasReturnValue = false;
  for (TFieldIterator<FProperty> params(udelegate); params; ++params) {
    check(!hasReturnValue);
    auto param = *params;
...
static void addRef(FProperty *inProp, FString &outString) {
  if (inProp->IsA<FNumericProperty>() || inProp->IsA<FEnumProperty>() || inProp->IsA<FObjectProperty>() || inProp->IsA<FBoolProperty>()) {
...
bool FHaxeGenerator::writeWithModifiers(const FString &inName, FProperty *inProp, FString &outType) {
...
bool FHaxeGenerator::writeBasicWithModifiers(const FString &inName, FProperty *inProp, FString &outType) {
  // TODO support basic types' modifiers
  outType += inName;
  return true;
}

static bool canBuildTArrayProp(FString inInner, FProperty *inProp) {
  ...
  if (inInner.Contains(TEXT("FStaticMeshComponentLODInfo"), ESearchCase::CaseSensitive, ESearchDir::FromStart)) {
    return false;
  } else if (inProp->IsA<FNumericProperty>() && CastField<FNumericProperty>(inProp)->GetIntPropertyEnum() != nullptr) {
...
bool FHaxeGenerator::isReadOnly(FProperty* inProp) {
  if (inProp->HasAnyPropertyFlags(CPF_ConstParm)) {
    return true;
  }
  if (inProp->IsA<FStructProperty>()) {
    auto prop = CastField<FStructProperty>(inProp);
...
bool FHaxeGenerator::upropType(FProperty* inProp, FString &outType) {
  if (inProp->ArrayDim > 1) {
    return false;
  }
  ...
  if (inProp->IsA<FStructProperty>()) {
  ...
  } else if (inProp->IsA<FObjectProperty>()) {
    if (inProp->IsA<FClassProperty>()) {
      auto prop = CastField<FClassProperty>(inProp);
    ...
    auto prop = CastField<FObjectProperty>(inProp);
    ...
    return writeWithModifiers(descr->haxeType.toString(), inProp, outType);
  } else if (inProp->IsA<FWeakObjectProperty>()) {
    auto prop = CastField<FWeakObjectProperty>(inProp);
    auto descr = m_haxeTypes.getDescriptor(prop->PropertyClass);
    if (descr == nullptr) {
      UE_LOG(LogHaxeExtern, Warning, TEXT("(uclass) TYPE NOT SUPPORTED: %s"), *prop->PropertyClass->GetName());
      return false;
    ...
    } else if (inProp->IsA<FDoubleProperty>()) {
    ...
    return writeWithModifiers(descr->haxeType.toString(), inProp, outType);
  } else if (inProp->IsA<FBoolProperty>()) {
  ...
  } else if (inProp->IsA<FTextProperty>()) {
    ...
    return false;
  } else if (inProp->IsA<FMapProperty>()) {
    auto prop = CastField<FMapProperty>(inProp);
    FString keyProp;
    FString valProp;
  ...
  } else if (inProp->IsA<FSetProperty>()) {
    auto prop = CastField<FSetProperty>(inProp);
    FString element;
